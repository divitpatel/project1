...
export class BillingActions {
  constructor(private ngRedux: NgRedux<IAppState>, private billingService: BillingService, 
    private router: Router, private msgService: MessagingService) { }
  ...
  fetchBillingSummary(nextPageParams) {
    ...
    this.billingService.fetchBilling(nextPageParams)
      .subscribe((response) => {
        let billingSummary = JSON.parse(response._body);
        if (!billingSummary)
