  ...
  createReport(reportCriteria) {
    ...
    this.reportService.createReport(reportCriteria)
      .subscribe((reportCriteria) => {
        ...
        this.getReports({userId: this.userId, pageNumber: 1, size: 20, sort:"-submittedOn" });
      },
      (err) => {
        console.log('error while creating a report', err);
        this.getReports({userId: this.userId, pageNumber: 1, size: 20, sort:"-submittedOn" });
        // this.msgService.error(`Unable to create ${reportCriteria.reportType} report, ${reportCriteria.title} now. Please try again later.`);
        this.ngRedux.dispatch({
  ...
  getReports(params) {
    ...
    this.reportService.getReports(params)
      .subscribe(reportsList => {
        let reportSummary = (reportsList._body!="") ? JSON.parse(reportsList._body) : null;
        if (!reportSummary){
