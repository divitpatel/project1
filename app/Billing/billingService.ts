...
export class BillingService {
  ...
  fetchBilling(nextPageParams) {
    ...
    let url = `${this.BASE_URL}/bob/billing/summary/${nextPageParams.userId}?page=${nextPageParams.pageNumber}&size=20&sort=${nextPageParams.sort}`;
  ...
  clientSearch({ taxId, searchBy, searchTerm }) {
    let url = `${this.BASE_URL}/client/search/${taxId}?filter=${searchBy}::${searchTerm}`;
